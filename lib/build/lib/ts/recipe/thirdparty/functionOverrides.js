...
var getFunctionOverrides = function (recipeId, onHandleEvent) {
    return function (originalImp) { return (__assign(__assign({}, originalImp), { signInAndUp: function (input) {
            ...
            return originalImp.setStateAndOtherInfoToStorage({
                state: __assign(__assign({}, input.state), { rid: recipeId, oauth2LoginChallenge: loginChallenge, redirectToPath: getRedirectToPathFromURL() }),
                userContext: input.userContext,
            });
        } })); };
};
