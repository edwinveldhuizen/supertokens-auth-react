...
function chooseComponentBasedOnFirstFactors(firstFactors, routeComponents) {
        ...
        if (firstFactors.every(function (factor) { return factorsProvided.includes(factor); })) {
            ...
            if (matchingComp) {
                ...
                if (firstFactors.length === factorsProvided.length) {
                    logDebugMessage("Rendering ".concat(rid, " because it matches factors: ").concat(firstFactors, " exactly"));
    ...
    */
    ...
    if ([FactorIds.OTP_PHONE, FactorIds.OTP_EMAIL, FactorIds.LINK_PHONE, FactorIds.LINK_EMAIL].some(function (pwlessFactorId) {
