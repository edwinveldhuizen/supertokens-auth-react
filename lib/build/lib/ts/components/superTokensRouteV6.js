...
function getSuperTokensRoutesForReactRouterDomV6(_a) {
    ...
    var routes = Object.values(recipeList.reduce(function (routes, recipe) {
        ...
        Object.keys(pathsToFeatureComponentWithRecipeIdMap).forEach(function (path) {
            ...
            if (!(path in routes)) {
                routes[path] = (jsx(Route, { path: pathForRouter, element: jsx(RoutingComponent, { getReactRouterDomWithCustomHistory: getReactRouterDomWithCustomHistory, preBuiltUIList: recipeList, path: path }) }, "st-".concat(path)));
            }
        });
        return routes;
    }, {}));
    if (!SuperTokens.getInstanceOrThrow().disableAuthRoute &&
        ...
        routes.push(jsx(Route, { path: getPathForRouter(basePath, path), element: jsx(RoutingComponent, { getReactRouterDomWithCustomHistory: getReactRouterDomWithCustomHistory, preBuiltUIList: recipeList, path: path }) }, "st-/auth"));
    }
    return routes;
}
